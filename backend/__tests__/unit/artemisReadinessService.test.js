...
describe('Artemis WP-A readiness aggregation', () => {
  ...
  it('returns LEGACY_ADVISORY readiness with executionEligible false', async () => {
    ...
    expect(readiness.evidence.readiness).toBe('ARTEMIS_CONSUMABLE');
    expect(readiness.evidence.artemisConsumable).toBe(true);
    expect(readiness.evidence.decisionEligible).toBe(false);
    expect(readiness.evidence.executionEligible).toBe(false);
    expect(readiness.evidence.ingestionOwner).toBe('artemisEvidenceIngestionService');
    ...
    expect(readiness.executionEligible).toBe(false);
    expect(readiness.blockers.some((b) => b.code === 'evidence_not_connected')).toBe(false);
    expect(readiness.blockers.some((b) => b.code === 'orchestration_not_active')).toBe(true);
    ...
    expect(readiness.catalog.agents.find((a) => a.key === 'pattern').ingestionReadiness).toBe('ARTEMIS_CONSUMABLE');
    ...
    expect(readiness.catalog.agents.find((a) => a.key === 'order').ingestionReadiness).toBe('ARTEMIS_CONSUMABLE');
    expect(readiness.catalog.agents.find((a) => a.key === 'optimization').readiness).toBe('NOT_APPLICABLE');
    expect(readiness.catalog.agents.find((a) => a.key === 'optimization').ingestionReadiness).toBe('NOT_APPLICABLE');
    expect(readiness.catalog.agents.find((a) => a.key === 'optimization').authority).toBe('not_applicable');
    expect(readiness.limitations).toContain('artemis_stage4_readonly_ingestion_advisory_only');
