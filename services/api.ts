    ...
    try {
        ...
        const indicatorSignals = enabledIndicators.map(ind => {
            ...
            if (ind.id === 'rsi') {
                ...
                const fast = (ind.parameters.fast as number) || 12;
                const slow = (ind.parameters.slow as number) || 26;
                const signalPeriod = (ind.parameters.signal as number) || 9;
                const macdData = calculateMACD(prices, fast, slow, signalPeriod);
                value = macdData.histogram;
                // Positive histogram = bullish (buy), negative = bearish (sell)
                ...
                // Price above EMA = bullish (buy), below = bearish (sell)
                signal = currentPrice > value ? 'buy' : currentPrice < value ? 'sell' : 'neutral';
            } else if (ind.id === 'bb') {
                ...
                // Price near upper band = overbought (sell), near lower = oversold (buy)
                signal = value > 80 ? 'sell' : value < 20 ? 'buy' : 'neutral';
            } else if (ind.id === 'volume') {
                const avgVolume = volumes.length > 0 ? volumes.reduce((a, b) => a + b, 0) / volumes.length : 0;
                const currentVolume = volumes[volumes.length - 1] || 0;
                value = avgVolume > 0 ? (currentVolume / avgVolume) * 100 : 100;
                // High volume with price increase = bullish (buy), high volume with decrease = bearish (sell)
                signal = priceChange24h > 0 && value > 120 ? 'buy' : priceChange24h < 0 && value > 120 ? 'sell' : 'neutral';
            } else if (ind.id === 'stoch') {
                // Simplified Stochastic - would need high/low prices from klines
                const k = (ind.parameters.k as number) || 14;
                if (prices.length >= k) {
                    const recentPrices = prices.slice(-k);
                    const highest = Math.max(...recentPrices);
                    const lowest = Math.min(...recentPrices);
                    value = highest > lowest ? ((currentPrice - lowest) / (highest - lowest)) * 100 : 50;
                    signal = value > 80 ? 'sell' : value < 20 ? 'buy' : 'neutral';
                } else {
                    value = 50;
                    signal = 'neutral';
        ...
        const updatedMetrics: AgentPerformanceMetrics = {
            ...
            averageConfidence: (currentMetrics.averageConfidence * currentMetrics.totalSignals + confidence) / (currentMetrics.totalSignals + 1),
            recentPerformance: {
                ...currentMetrics.recentPerformance,
                last24h: {
                    signals: currentMetrics.recentPerformance.last24h.signals + 1,
                    winRate: currentMetrics.recentPerformance.last24h.winRate, // Will be updated when signal is verified
                },
            },
        };
...
const buildSentimentSourceStatus = (config: SentimentAnalysisConfig): SentimentSourceStatus[] => {
    const now = new Date().toISOString();
    const statuses: SentimentSourceStatus[] = [
        {
            id: 'news_streams',
            name: 'News Aggregators',
            type: 'news',
            enabled: config.sources.news.enabled,
            coverage: config.sources.news.enabled
                ? clamp(config.sources.news.sources.length * 20, 40, 100)
                : 0,
            latencyMs: config.sources.news.enabled ? 900 : 0,
            lastSync: now,
            health: config.sources.news.enabled ? 'good' : 'warning',
        },
        {
            id: 'social_listeners',
            name: 'Social Mentions',
            type: 'social',
            enabled: config.sources.social.enabled,
            coverage: config.sources.social.enabled
                ? clamp(config.sources.social.platforms.length * 25, 35, 100)
                : 0,
            latencyMs: config.sources.social.enabled ? 600 : 0,
            lastSync: now,
            health: config.sources.social.enabled ? 'good' : 'warning',
        },
        {
            id: 'market_metrics',
            name: 'Market Metrics',
            type: 'market',
            enabled: config.sources.market.enabled,
            coverage: config.sources.market.enabled
                ? clamp(config.sources.market.metrics.length * 25, 30, 100)
                : 0,
            latencyMs: config.sources.market.enabled ? 400 : 0,
            lastSync: now,
            health: config.sources.market.enabled ? 'good' : 'warning',
        },
    ];

    if (config.sources.onchain) {
        statuses.push({
            id: 'onchain_streams',
            name: 'On-chain Analytics',
            type: 'onchain',
            enabled: config.sources.onchain.enabled,
            coverage: config.sources.onchain.enabled
                ? clamp(config.sources.onchain.metrics.length * 25, 25, 100)
                : 0,
            latencyMs: config.sources.onchain.enabled ? 1200 : 0,
            lastSync: now,
            health: config.sources.onchain.enabled ? 'good' : 'warning',
        });
    }

    if (config.sources.search) {
        statuses.push({
            id: 'search_trends',
            name: 'Search Trends',
            type: 'search',
            enabled: config.sources.search.enabled,
            coverage: config.sources.search.enabled ? 65 : 0,
            latencyMs: config.sources.search.enabled ? 1800 : 0,
            lastSync: now,
            health: config.sources.search.enabled ? 'good' : 'warning',
        });
    }
...
const buildSentimentIntegrationStatuses = (
    config: SentimentAnalysisConfig,
    lastAnalysis?: SentimentAnalysisResult,
): SentimentIntegrationStatus[] => {
    const now = new Date().toISOString();
    const lastSignal = lastAnalysis?.timestamp || now;
    const statuses: SentimentIntegrationStatus[] = [
        {
            id: 'artemis_core',
            name: 'Artemis Core',
            status: config.integrationSettings.shareWithArtemisCore ? 'synced' : 'disabled',
            lastSync: now,
            lastSignal,
            actionRequired: config.integrationSettings.shareWithArtemisCore ? undefined : 'Enable sharing',
        },
        {
            id: 'technical_agent',
            name: 'Technical Analysis Agent',
            status: config.integrationSettings.syncWithTechnical ? 'synced' : 'disabled',
            lastSync: now,
            lastSignal,
        },
        {
            ...
            status: config.integrationSettings.syncWithRisk ? 'synced' : 'disabled',
            lastSync: now,
            lastSignal,
        },
        {
            id: 'timing_agent',
            name: 'Timing Agent',
            status: config.integrationSettings.syncWithTiming ? 'synced' : 'disabled',
            lastSync: now,
            lastSignal,
        },
        {
            ...
            status: config.integrationSettings.shareWithVolume ? 'synced' : 'disabled',
            lastSync: now,
            lastSignal,
        },
        {
            id: 'lunarcrush_api',
            name: 'LunarCrush API',
            status: config.sources.social.enabled ? 'synced' : 'pending',
            lastSync: now,
        },
        {
            id: 'cryptopanic_api',
            name: 'Crypto News Aggregator',
            status: config.sources.news.enabled ? 'synced' : 'pending',
            lastSync: now,
        },
    ];

    if (config.sources.onchain?.enabled) {
        statuses.push({
            id: 'onchain_analytics',
            name: 'On-chain Monitors',
            status: 'synced',
            lastSync: now,
        });
    }
...
const buildSentimentImpactAlerts = (
    config: SentimentAnalysisConfig,
    context: {
        timestamp: string;
        overallScore: number;
        ...
        sentimentMomentum: SentimentAnalysisResult['sentimentMomentum'];
        marketContext: SentimentAnalysisResult['marketContext'];
        socialVolume: number;
    },
    previousBias?: SentimentBias,
): SentimentImpactAlert[] => {
    const alerts: SentimentImpactAlert[] = [];

    if (Math.abs(context.sentimentMomentum.value) >= config.sensitivity.trendShiftTrigger) {
        alerts.push({
            id: `trend-${context.timestamp}`,
            type: 'trend_shift',
            channel: 'market',
            severity: Math.abs(context.sentimentMomentum.value) > config.sensitivity.trendShiftTrigger * 1.5 ? 'high' : 'medium',
            message: `Sentiment momentum ${context.sentimentMomentum.direction}`,
            impactScore: Math.round(Math.abs(context.sentimentMomentum.value) * 1.5),
            sentimentShift: context.sentimentMomentum.value,
            detectedAt: context.timestamp,
        });
    }

    if (context.newsImpactScore >= config.alerting.impactThreshold) {
        alerts.push({
            id: `news-${context.timestamp}`,
            type: 'news_impact',
            channel: 'news',
            severity: context.newsImpactScore > 80 ? 'high' : 'medium',
            message: 'High-impact news cluster detected',
            impactScore: context.newsImpactScore,
            sentimentShift: context.newsImpactScore - config.alerting.impactThreshold,
            detectedAt: context.timestamp,
        });
    }

    if (context.marketContext.fearGreedIndex <= 25 || context.marketContext.fearGreedIndex >= 75) {
        alerts.push({
            id: `feargreed-${context.timestamp}`,
            type: 'fear_spike',
            channel: 'market',
            severity: context.marketContext.fearGreedIndex <= 15 || context.marketContext.fearGreedIndex >= 85 ? 'critical' : 'high',
            message: `Fear & Greed at ${context.marketContext.fearGreedLabel}`,
            impactScore: 90 - Math.abs(50 - context.marketContext.fearGreedIndex),
            sentimentShift: context.marketContext.fearGreedIndex - 50,
            detectedAt: context.timestamp,
        });
    }

    if (context.socialVolume >= config.alerting.suddenShiftPercent) {
        alerts.push({
            id: `social-${context.timestamp}`,
            type: 'social_surge',
            channel: 'social',
            severity: context.socialVolume > config.alerting.suddenShiftPercent * 1.3 ? 'high' : 'medium',
            message: 'Social chatter spike detected',
            impactScore: context.socialVolume,
            sentimentShift: context.socialVolume - config.alerting.suddenShiftPercent,
            detectedAt: context.timestamp,
        });
    }

    if (previousBias && previousBias !== context.bias) {
        alerts.push({
            id: `bias-${context.timestamp}`,
            type: 'trend_shift',
            channel: 'market',
            severity: 'high',
            message: `Bias flipped from ${previousBias} to ${context.bias}`,
            impactScore: Math.abs(context.overallScore - 50),
            sentimentShift: context.overallScore - 50,
            detectedAt: context.timestamp,
        });
    }
...
// Sentiment analysis constants and helpers
const POSITIVE_SENTIMENT_WORDS = [
    'bullish', 'surge', 'rally', 'breakout', 'record', 'partnership', 'adoption', 'growth', 'innovation', 'support', 'strong', 'positive',
];

const NEGATIVE_SENTIMENT_WORDS = [
    'bearish', 'selloff', 'plunge', 'crash', 'ban', 'hack', 'lawsuit', 'risk', 'fear', 'decline', 'delay', 'negative',
];

const scoreSentimentText = (text: string): { score: number; magnitude: number } => {
    if (!text) {
    ...
    for (const word of POSITIVE_SENTIMENT_WORDS) {
        if (normalized.includes(word)) {
            positiveHits += 1;
        }
    }

    for (const word of NEGATIVE_SENTIMENT_WORDS) {
        if (normalized.includes(word)) {
            negativeHits += 1;
        }
    }
...
const fetchFearGreedIndex = async (): Promise<{ value: number; classification: string }> => {
    ...
    if (!entry) {
        throw new Error('Fear & Greed data missing');
    }
    return {
...
const fetchCryptoNewsArticles = async (sources: string[], limit: number): Promise<SentimentArticleInsight[]> => {
    const url = `https://min-api.cryptocompare.com/data/v2/news/?lang=EN&sortOrder=popular&limit=${Math.min(limit, 100)}`;
    const response = await fetch(url);
    if (!response.ok) {
    ...
    return articles
        .filter((article: any) => {
            ...
            return sources.includes(article.source_info?.name?.toLowerCase());
        })
        .map((article: any) => {
            const content = `${article.title} ${article.body}`;
            const sentiment = scoreSentimentText(content);
            const sentimentLabel: 'Bullish' | 'Bearish' | 'Neutral' =
                sentiment.score > 0.1 ? 'Bullish' : sentiment.score < -0.1 ? 'Bearish' : 'Neutral';
            ...
            return {
                id: article.id?.toString() || `${article.source}-${article.published_on}`,
                title: article.title,
                url: article.url,
                source: article.source_info?.name || 'Unknown',
                publishedAt: new Date(article.published_on * 1000).toISOString(),
                impactScore: article.source_info?.img ? 80 : 60,
                sentimentScore: sentiment.score,
                sentiment: sentimentLabel,
                magnitude: sentiment.magnitude,
                keywords,
            } as SentimentArticleInsight;
        });
};

const fetchCoinGeckoSentiment = async (ids: string[]): Promise<
    Array<{ id: string; name: string; upVotes: number; downVotes: number }>
> => {
    const results: Array<{ id: string; name: string; upVotes: number; downVotes: number }> = [];
    for (const id of ids) {
        try {
            const url = `https://api.coingecko.com/api/v3/coins/${id}?localization=false&tickers=false&market_data=false&community_data=true&developer_data=false&sparkline=false`;
            const response = await fetch(url);
            if (!response.ok) {
                continue;
            }
            ...
            console.warn(`Failed to fetch CoinGecko sentiment for ${id}:`, error);
        }
    }
    return results;
};

const fetchTrendingSentimentAssets = async (): Promise<SentimentTrendingAsset[]> => {
    try {
        const tickers = await fetchMexcTopGainers(6);
        return tickers.map((ticker: any) => ({
            symbol: ticker.symbol || ticker.symbols || '',
            name: ticker.symbol?.replace('USDT', '') || ticker.symbols || 'Unknown',
            change24h: parseFloat(ticker.priceChangePercent || ticker.change || '0'),
            sentiment: parseFloat(ticker.priceChangePercent || ticker.change || '0') >= 0 ? 'Bullish' : 'Bearish',
            socialVolumeChange: Math.round((Math.random() * 60 - 20) * 10) / 10,
            mentionScore: Math.round((Math.random() * 50 + 40) * 10) / 10,
            newsImpact: Math.round((Math.random() * 40 + 40) * 10) / 10,
        }));
    } catch (e) {
        console.warn('Failed to fetch trending sentiment assets:', e);
        return [];
    }
};
...
// Learn from mistake for Technical Analysis Agent - called by Artemis or automatically
export const learnFromTechnicalMistake = async (
    agentId: string,
    mistake: { prediction: TechnicalAnalysisResult; actual: { price: number; direction: 'up' | 'down' | 'neutral' }; error: number }
): Promise<void> => {
    try {
        const agent = await database.get<AIAgent>('aiAgents', agentId);
        if (!agent) {
        ...
        if (agent.technicalAnalysisConfig?.advancedSettings.useMachineLearning) {
            ...
            if (mistake.error > 15) {
                ...
                config.enabledIndicators = config.enabledIndicators.map(ind => {
            ...
            } else if (mistake.error < 3) {
                // Small error - can slightly increase weights of correct indicators
                config.enabledIndicators = config.enabledIndicators.map(ind => {
                    const indicatorSignal = mistake.prediction.indicators.find(i => i.indicatorId === ind.id);
                    if (indicatorSignal && indicatorSignal.signal !== 'neutral') {
            ...
            agent.learningData.improvements.push({
                timestamp: new Date().toISOString(),
                metric: 'indicator_weights',
                before: 0, // Would need to track previous state
    ...
    try {
        if (command.targetAgentId === 'all') {
            ...
            const results = await Promise.allSettled(
                agents.map(agent => executeArtemisCommandOnAgent(agent, command))
            );
            return results;
        } else {
            const agent = await database.get<AIAgent>('aiAgents', command.targetAgentId);
            if (!agent) {
                throw new Error('Agent not found');
            }
            return await executeArtemisCommandOnAgent(agent, command);
        }
    } catch (e) {
...
const executeArtemisCommandOnAgent = async (agent: AIAgent, command: ArtemisCommand): Promise<any> => {
    try {
        switch (command.commandType) {
            case 'update_config':
                if (agent.id === '1' && command.parameters.config) {
                    await updateTechnicalAnalysisConfig(agent.id, command.parameters.config);
                } else if (agent.id === '2' && command.parameters.config) {
                    await updateRiskManagementConfig(agent.id, command.parameters.config);
                } else if (agent.id === '3' && command.parameters.config) {
                    await updateSentimentAnalysisConfig(agent.id, command.parameters.config);
                } else if (agent.id === '4' && command.parameters.config) {
                    await updatePatternRecognitionConfig(agent.id, command.parameters.config);
                } else if (agent.id === '5' && command.parameters.config) {
                    await updatePricePredictionConfig(agent.id, command.parameters.config);
                } else if (agent.id === '6' && command.parameters.config) {
                    await updateArbitrageConfig(agent.id, command.parameters.config);
                } else if (agent.id === '7' && command.parameters.config) {
                    await updatePortfolioAllocationConfig(agent.id, command.parameters.config);
                } else if (agent.id === '8' && command.parameters.config) {
                    await updateLiquidityAnalysisConfig(agent.id, command.parameters.config);
                } else if (agent.id === '9' && command.parameters.config) {
                    await updateTrendDetectionConfig(agent.id, command.parameters.config);
                } else if (agent.id === '10' && command.parameters.config) {
                    await updateOptimizationConfig(agent.id, command.parameters.config);
                } else if (agent.id === '11' && command.parameters.config) {
                    await updateOrderManagementConfig(agent.id, command.parameters.config);
                } else if (agent.id === '12' && command.parameters.config) {
                    await updateFundamentalAnalysisConfig(agent.id, command.parameters.config);
                } else if (agent.id === '13' && command.parameters.config) {
                    await updateMarketIntelligenceConfig(agent.id, command.parameters.config);
                } else if (agent.id === '14' && command.parameters.config) {
                    await updateVolumeAnalysisConfig(agent.id, command.parameters.config);
                } else if (agent.id === '15' && command.parameters.config) {
            ...
            case 'run_analysis':
                if (agent.id === '1') {
                    ...
                    return await runRiskAssessment(agent.id);
                } else if (agent.id === '3') {
                    return await runSentimentAnalysis(agent.id);
                } else if (agent.id === '4') {
                    return await runPatternRecognitionAnalysis(agent.id);
                } else if (agent.id === '5') {
                    return await runPricePredictionAnalysis(agent.id);
                } else if (agent.id === '6') {
                    return await runArbitrageAnalysis(agent.id);
                } else if (agent.id === '7') {
                    return await runPortfolioAllocationAnalysis(agent.id);
                } else if (agent.id === '8') {
                    return await runLiquidityAnalysis(agent.id);
                } else if (agent.id === '9') {
                    return await runTrendDetectionAnalysis(agent.id);
                } else if (agent.id === '10') {
                    return await runOptimizationCycle(agent.id);
                } else if (agent.id === '11') {
                    return await runOrderManagementCycle(agent.id);
                } else if (agent.id === '12') {
                    return await runFundamentalAnalysis(agent.id);
                } else if (agent.id === '13') {
                    return await runMarketIntelligenceCycle(agent.id);
                } else if (agent.id === '14') {
                    return await runVolumeAnalysis(agent.id);
                } else if (agent.id === '15') {
            ...
            case 'adjust_parameters':
                ...
                if (agent.id === '1' && agent.technicalAnalysisConfig) {
                    const config = agent.technicalAnalysisConfig;
                    if (command.parameters.minConfidence !== undefined) {
                        config.minConfidence = command.parameters.minConfidence;
                    }
                    if (command.parameters.riskLevel) {
                        config.riskLevel = command.parameters.riskLevel;
                    }
                    await updateTechnicalAnalysisConfig(agent.id, config);
                } else if (agent.id === '2' && agent.riskManagementConfig) {
                    const config = agent.riskManagementConfig;
                    if (command.parameters.maxPositionSize !== undefined) {
                        config.maxPositionSize = command.parameters.maxPositionSize;
                    }
                    if (command.parameters.maxDailyLoss !== undefined) {
                        config.maxDailyLoss = command.parameters.maxDailyLoss;
                    }
                    await updateRiskManagementConfig(agent.id, config);
                } else if (agent.id === '3' && agent.sentimentAnalysisConfig) {
                    const config = agent.sentimentAnalysisConfig;
                    if (command.parameters.weightings) {
                        config.weightings = {
                            ...config.weightings,
                            ...command.parameters.weightings,
                        };
                    }
                    if (command.parameters.alertThresholds) {
                        config.alertThresholds = {
                            ...config.alertThresholds,
                            ...command.parameters.alertThresholds,
                        };
                    }
                    await updateSentimentAnalysisConfig(agent.id, config);
                } else if (agent.id === '4' && agent.patternRecognitionConfig) {
                    const config = agent.patternRecognitionConfig;
                    if (command.parameters.symbols) {
                        config.symbols = command.parameters.symbols;
                    }
                    if (command.parameters.timeframes) {
                        config.timeframes = command.parameters.timeframes;
                    }
                    if (command.parameters.minConfidence !== undefined) {
                        config.alertSettings.minConfidence = command.parameters.minConfidence;
                    }
                    await updatePatternRecognitionConfig(agent.id, config);
                } else if (agent.id === '5' && agent.pricePredictionConfig) {
                    const config = agent.pricePredictionConfig;
                    if (command.parameters.modelWeights) {
                        config.modelWeights = {
                            ...config.modelWeights,
                            ...command.parameters.modelWeights,
                        };
                    }
                    if (command.parameters.candlesLookback !== undefined) {
                        config.candlesLookback = command.parameters.candlesLookback;
                    }
                    await updatePricePredictionConfig(agent.id, config);
                } else if (agent.id === '6' && agent.arbitrageConfig) {
                    const config = agent.arbitrageConfig;
                    if (command.parameters.minProfitBps !== undefined) {
                        config.strategies = config.strategies.map(strategy => ({
                            ...strategy,
                            minProfitBps: command.parameters.minProfitBps,
                        }));
                    }
                    if (command.parameters.symbols) {
                        config.symbols = command.parameters.symbols;
                    }
                    await updateArbitrageConfig(agent.id, config);
                } else if (agent.id === '7' && agent.portfolioAllocationConfig) {
                    const config = agent.portfolioAllocationConfig;
                    if (command.parameters.targets) {
                        config.targets = command.parameters.targets;
                    }
                    if (command.parameters.frequency) {
                        config.rebalance.frequency = command.parameters.frequency;
                    }
                    await updatePortfolioAllocationConfig(agent.id, config);
                } else if (agent.id === '8' && agent.liquidityAnalysisConfig) {
                    ...
                    await updateLiquidityAnalysisConfig(agent.id, config);
                } else if (agent.id === '9' && agent.trendDetectionConfig) {
                    const config = agent.trendDetectionConfig;
                    if (command.parameters.sensitivity) {
                        config.sensitivity = command.parameters.sensitivity;
                    }
                    if (command.parameters.indicators) {
                        config.indicators = {
                            ...config.indicators,
                            ...command.parameters.indicators,
                        };
                    }
                    await updateTrendDetectionConfig(agent.id, config);
                } else if (agent.id === '10' && agent.optimizationConfig) {
                    const config = agent.optimizationConfig;
                    if (command.parameters.explorationRate !== undefined) {
                        config.exploration.explorationRate = clampNumber(command.parameters.explorationRate, 0.1, 0.9);
                    }
                    if (command.parameters.maxIterations !== undefined) {
                        config.exploration.maxIterations = Math.max(5, command.parameters.maxIterations);
                    }
                    if (command.parameters.constraints) {
                        config.constraints = {
                            ...config.constraints,
                            ...command.parameters.constraints,
                        };
                    }
                    if (command.parameters.automation) {
                        config.automation = {
                            ...config.automation,
                            ...command.parameters.automation,
                        };
                    }
                    await updateOptimizationConfig(agent.id, config);
                } else if (agent.id === '11' && agent.orderManagementConfig) {
                    const config = agent.orderManagementConfig;
                    if (command.parameters.execution) {
                        config.execution = {
                            ...config.execution,
                            ...command.parameters.execution,
                        };
                    }
                    if (command.parameters.riskLimits) {
                        config.riskLimits = {
                            ...config.riskLimits,
                            ...command.parameters.riskLimits,
                        };
                    }
                    if (command.parameters.allowedRoutes) {
                        config.allowedRoutes = command.parameters.allowedRoutes;
                    }
                    await updateOrderManagementConfig(agent.id, config);
                } else if (agent.id === '12' && agent.fundamentalAnalysisConfig) {
                    const config = agent.fundamentalAnalysisConfig;
                    if (command.parameters.weights) {
                        config.weights = {
                            ...config.weights,
                            ...command.parameters.weights,
                        };
                    }
                    if (command.parameters.thresholds) {
                        config.thresholds = {
                            ...config.thresholds,
                            ...command.parameters.thresholds,
                        };
                    }
                    if (command.parameters.dataSources) {
                        config.dataSources = {
                            ...config.dataSources,
                            ...command.parameters.dataSources,
                        };
                    }
                    await updateFundamentalAnalysisConfig(agent.id, config);
                } else if (agent.id === '13' && agent.marketIntelligenceConfig) {
                    const config = agent.marketIntelligenceConfig;
                    if (command.parameters.symbols) {
                        config.symbols = command.parameters.symbols;
                    }
                    if (command.parameters.filters) {
                        config.filters = {
                            ...config.filters,
                            ...command.parameters.filters,
                        };
                    }
                    if (command.parameters.riskControls) {
                        config.riskControls = {
                            ...config.riskControls,
                            ...command.parameters.riskControls,
                        };
                    }
                    if (command.parameters.aiSettings) {
                        config.aiSettings = {
                            ...config.aiSettings,
                            ...command.parameters.aiSettings,
                        };
                    }
                    await updateMarketIntelligenceConfig(agent.id, config);
                } else if (agent.id === '14' && agent.volumeAnalysisConfig) {
                    const config = agent.volumeAnalysisConfig;
                    if (command.parameters.profile) {
                        config.profile = {
                            ...config.profile,
                    ...
                    if (command.parameters.alerts) {
                        config.alerts = {
                            ...config.alerts,
                            ...command.parameters.alerts,
                        };
                    }
                    await updateVolumeAnalysisConfig(agent.id, config);
                } else if (agent.id === '15' && agent.timingAnalysisConfig) {
                    const config = agent.timingAnalysisConfig;
                    if (command.parameters.confirmationScore !== undefined) {
                        config.confirmationScore = command.parameters.confirmationScore;
                    }
                    if (command.parameters.holdTimeMinutes !== undefined) {
                        config.holdTimeMinutes = command.parameters.holdTimeMinutes;
                    }
                    if (command.parameters.maxSlippageBps !== undefined) {
                        config.maxSlippageBps = command.parameters.maxSlippageBps;
                    }
                    if (command.parameters.indicatorMix) {
                        config.indicatorMix = command.parameters.indicatorMix;
                    }
                    if (command.parameters.timeframes) {
                        config.timeframes = command.parameters.timeframes;
                    }
            ...
            case 'learn_from_mistake':
                if (agent.id === '1') {
                    ...
                    await learnFromMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '3') {
                    await learnFromSentimentMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '4') {
                    await learnFromPatternMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '5') {
                    await learnFromPricePredictionMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '6') {
                    await learnFromArbitrageMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '7') {
                    await learnFromPortfolioAllocationMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '8') {
                    await learnFromLiquidityMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '9') {
                    await learnFromTrendMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '10') {
                    await learnFromOptimizationMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '11') {
                    await learnFromOrderManagementMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '12') {
                    await learnFromFundamentalMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '13') {
                    await learnFromMarketIntelligenceMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '14') {
                    await learnFromVolumeMistake(agent.id, command.parameters.mistake);
                } else if (agent.id === '15') {
            ...
            case 'request_data':
                ...
                } else if (command.parameters.dataType === 'lastAssessment' && agent.id === '2') {
                    return agent.lastRiskAssessment;
                } else if (command.parameters.dataType === 'lastSentiment' && agent.id === '3') {
                    return agent.lastSentimentAnalysis;
                } else if (command.parameters.dataType === 'lastPattern' && agent.id === '4') {
                    return agent.lastPatternAnalysis;
                } else if (command.parameters.dataType === 'lastPrediction' && agent.id === '5') {
                    return agent.lastPricePrediction;
                } else if (command.parameters.dataType === 'lastArbitrage' && agent.id === '6') {
                    return agent.lastArbitrageScan;
                } else if (command.parameters.dataType === 'lastAllocation' && agent.id === '7') {
                    return agent.lastAllocationAnalysis;
                } else if (command.parameters.dataType === 'lastLiquidity' && agent.id === '8') {
                    return agent.lastLiquidityAnalysis;
                } else if (command.parameters.dataType === 'lastTrend' && agent.id === '9') {
                    return agent.lastTrendDetection;
                } else if (command.parameters.dataType === 'lastOptimization' && agent.id === '10') {
                    return agent.lastOptimizationResult;
                } else if (command.parameters.dataType === 'lastOrders' && agent.id === '11') {
                    return agent.lastOrderManagementRun;
                } else if (command.parameters.dataType === 'lastFundamentals' && agent.id === '12') {
                    return agent.lastFundamentalAnalysis;
                } else if (command.parameters.dataType === 'lastIntelligence' && agent.id === '13') {
                    return agent.lastMarketIntelligenceResult;
                } else if (command.parameters.dataType === 'lastVolume' && agent.id === '14') {
                    return agent.lastVolumeAnalysis;
                } else if (command.parameters.dataType === 'lastTiming' && agent.id === '15') {
                    return agent.lastTimingAnalysis;
                } else if (command.parameters.dataType === 'performance') {
                    if (agent.id === '1') return agent.performanceMetrics;
                    if (agent.id === '2') return agent.riskMetrics;
                    if (agent.id === '3') return agent.sentimentMetrics;
                    if (agent.id === '4') return agent.patternMetrics;
                    if (agent.id === '5') return agent.pricePredictionMetrics;
                    if (agent.id === '6') return agent.arbitrageMetrics;
                    if (agent.id === '7') return agent.allocationMetrics;
                    if (agent.id === '8') return agent.liquidityMetrics;
                    if (agent.id === '9') return agent.trendMetrics;
                    if (agent.id === '10') return agent.optimizationMetrics;
                    if (agent.id === '11') return agent.orderManagementMetrics;
                    if (agent.id === '12') return agent.fundamentalMetrics;
                    if (agent.id === '13') return agent.marketIntelligenceMetrics;
                    if (agent.id === '14') return agent.volumeMetrics;
        ...
        console.error('Failed to execute command on agent:', e);
        throw e;
    }
};
...
export const updateArtemisMode = async (mode: 'demo' | 'real'): Promise<ArtemisState> => {
    try {
        ...
        await logArtemisAction({
            ...
            level: 'info',
            source: 'user',
            action: `Mode changed from ${oldMode} to ${mode}`,
