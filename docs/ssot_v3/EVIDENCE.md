...
- **Security Check**:  
  ...
  - **Runtime verified (DH-P0-SECURITY-5, 2026-05-30):** production `auth-role` default — unauth → 401, admin → 200 (except `/stats/real-time` handler 500 pre-existing schema), user JWT → 403 on all read routes tested; `mark-processed` → 401 unauth, 400 empty payload with auth.  
- **Logging & Trace**:  
