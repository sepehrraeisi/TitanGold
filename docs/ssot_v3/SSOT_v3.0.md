...
| `dataHub.advanced.telegramPublisher` | Telegram Publisher | `TelegramPublisher.tsx` | *(planned)* `/api/v1/data-hub/telegram-publishers` | *(planned)* `telegram_publishers`, `publisher_delivery_history` | publisher worker | Partial | UI از `dataHub.advanced.*` (IndexedDB). Backend publisher API **ندارد** → GAP-016. Contract: `docs/ssot_v3/advanced/TELEGRAM_PUBLISHER_API_CONTRACT.md`. |
