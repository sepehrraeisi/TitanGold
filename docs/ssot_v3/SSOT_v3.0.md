...
| Module ID | Module | UI | API | DB | Worker | Status | Evidence |
|---|---|---|---|---|---|---|---|
| `dataHub.sources` | Data Sources | `DataSourcesPanel.tsx` | `/api/v1/data-sources` (+ `test-connection`) | `data_sources`, `collected_data` | ingestion workers, crawlers | Implemented | UI: `useDataSourcesQuery` + `services/dataSourcesApi.ts` → `GET/POST/PUT/DELETE/PATCH` روی `backend/routes/data-sources.js`؛ pagination در UI با `pagination.total/limit/page/hasNextPage`؛ GAP-008 بسته v3.0. سایر تب‌های DataHub (categories, pipeline, …) همچنان بخشی از state محلی دارند. |
| `dataHub.categories` | Data Categories | `CategoriesPanel.tsx` | `/api/v1/data-categories` | `data_categories`, روابط به `collected_data` | -- | Implemented | UI: `CategoriesPanel.tsx` – API: `backend/routes/data-categories.js` |
| `dataHub.pipeline` | Data Pipeline | `PipelinePanel.tsx` | `/api/v1/collected-data/*`، stats views | `collected_data`, `telegram_*` pipeline views | ETL/pipeline workers | Partial | UI: `PipelinePanel.tsx` – API: `backend/routes/collected-data.js`, `backend/routes/telegram.js` |
...
| `dataHub.logs` | Access Logs | `LogsPanel.tsx` | `/api/v1/collected-data/*` (logs), احتمالا views | `collected_data`, log tables | -- | Partial | UI: `LogsPanel.tsx` – API: `backend/routes/collected-data.js` |
...
| `dataHub.advanced.telegramPublisher` | Telegram Publisher | `TelegramPublisher.tsx` | `/api/v1/telegram/...` (publisher endpoints) | publisher config tables | publisher worker | Partial | UI: `TelegramPublisher.tsx` – API: `backend/routes/telegram.js` (بخش publisher) |
| `dataHub.advanced.automation` | Automation / Topic Routing | `AutomationTopics.tsx` | `/api/v1/topic-routing` | `topic_routing_rules`, `topic_routing_logs`, `collected_data` | topicRouter service | Implemented | UI: `AutomationTopics.tsx` – API: `backend/routes/topic-routing.js` |
