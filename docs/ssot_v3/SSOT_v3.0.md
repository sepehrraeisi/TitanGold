...
| `dataHub.advanced.telegramPublisher` | Telegram Publisher | `TelegramPublisher.tsx` | `/api/v1/data-hub/telegram-publishers` (CRUD, test, publish, history) | `telegram_publishers`, `publisher_delivery_history` | publisher worker | Implemented | GAP-016 Closed. `useTelegramPublishersQuery` + dry-run/confirm publish. Automation queue هنوز IndexedDB → subtab automation. |
