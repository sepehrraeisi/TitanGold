...
async function checkHealth() {
  ...
  logger.info('Health:', res.status, body.status);
...
async function checkSwagger() {
  ...
  logger.info('Swagger docs:', res.status);
...
async function checkExportsUnauthorized() {
  ...
  logger.info('Exports (expected 401/200 depending on auth):', res.status);
...
async function run() {
  try {
    logger.info('Running integration smoke tests against', baseUrl);
    await checkHealth();
    await checkSwagger();
