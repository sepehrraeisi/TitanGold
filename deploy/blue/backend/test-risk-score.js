...
async function testRiskAgent() {
  logger.info('🧪 Testing Risk Agent riskScore...\n');
  
  // Test 1: Small trade (should be low risk)
  logger.info('Test 1: Small Trade (100 USDT)');
  ...
  logger.info('Test 2: Large Trade (15,000 USDT)');
  ...
  logger.info(`   Test 1 riskScore: ${result1.riskScore} (should be <80)`);
  logger.info(`   Test 2 riskScore: ${result2.riskScore} (should be >=60, ideally >=80 for blocking)`);
  logger.info(`   Both have riskScore in root: ${result1.riskScore !== undefined && result2.riskScore !== undefined ? '✅' : '❌'}`);
  logger.info(`   Both have riskScore in _meta: ${result1._meta.riskScore !== undefined && result2._meta.riskScore !== undefined ? '✅' : '❌'}`);
