...
#
# Usage: Run via cron daily at 5:00 AM
# 0 5 * * * /usr/local/bin/titangold-daily-report.sh
#
...
REPORT="${REPORT}_$(date '+%Y-%m-%d %H:%M:%S')_%0A%0A"
...
if [ -n "$LATEST_BACKUP" ]; then
    ...
    if [ -n "$PREV_BACKUP" ] && [ "$PREV_BACKUP" != "$LATEST_BACKUP" ]; then
        ...
        if (( $(echo "$SIZE_DIFF_PERCENT > 50" | bc -l) )); then
            ...
            GROWTH_STATUS="⚠️ WARNING"
        elif (( $(echo "$SIZE_DIFF_PERCENT < -50" | bc -l) )); then
            GROWTH_STATUS="🔴 CRITICAL DROP"
        elif (( $(echo "$SIZE_DIFF_PERCENT < -25" | bc -l) )); then
            GROWTH_STATUS="⚠️ WARNING DROP"
        fi
        
        REPORT="${REPORT}  Growth: ${GROWTH_STATUS} ${SIZE_DIFF_PERCENT}%%0A"
    fi
    ...
    REPORT="${REPORT}  🔴 NO BACKUPS FOUND!%0A"
fi
...
REPORT="${REPORT}  RAM: ${RAM_STATUS} ${RAM_USED} / ${RAM_TOTAL} (${RAM_PERCENT}%)%0A"
...
REPORT="${REPORT}  API: ${BACKEND_API_STATUS} HTTP ${BACKEND_HTTP_CODE} (${BACKEND_RESPONSE_TIME}s)%0A"
...
if [ -n "$RECENT_ERRORS" ]; then
    REPORT="${REPORT}⚠️ *Recent Errors*%0A"
    # Escape errors for Telegram (just show count, not full text)
